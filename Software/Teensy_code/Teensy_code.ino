...
char temp_tx_buffer[COBS_BUFFER_SIZE]; //Temporary buffer for preparing packets immediately before transmission
char temp_rx_buffer[COBS_BUFFER_SIZE]; //Temporary buffer for preparing packets immediately before transmission
...
uint32_t ext_avg[] = {0, 0}; //Summing variable for performing rolling average on the external thermistor to denoise it
uint16_t ext_sample_counter[] = {0,0}; //Number of samples currently stored in ext_avg
...
long error;
long prev_error; //For calculating the derivative term
long integral;
float derivative;
uint32_t pid_clock; //Cpu cycle clock to measure PID time step
...
//  EEPROM.update(0,0);
  ...
  ////// Serial /////
  Serial1.begin(pin.SERIAL1_BAUD);
  Serial2.begin(pin.SERIAL2_BAUD);
  Serial3.begin(pin.SERIAL3_BAUD);

  Serial1.setRX(pin.SERIAL1_RX);
  Serial1.setTX(pin.SERIAL1_TX);
...
void moveToPosition(long final_position, bool backlash){
  ...
  uint16_t motor_stuck_attempt = 0; 
  pid_clock = ARM_DWT_CYCCNT; //Get current time in clock cycles
  digitalWriteFast(pin.MOTOR[0], LOW);
  digitalWriteFast(pin.MOTOR[1], LOW);
  ...
  while(encoder.read() != final_position){
    if(interval_timer > STUCK_MOTOR_INTERVAL || current_position != encoder.read()){
      ...
      if(control_out > 0){
        ...
        analogWrite(pin.MOTOR[0], motor_power);
        digitalWriteFast(pin.MOTOR[1], LOW);
      }
    }
  }
...
void calculatePID(long final_position){
  uint32_t power;
  
  deltaT = ARM_DWT_CYCCNT-pid_clock;
  pid_clock = ARM_DWT_CYCCNT;
  
  //Position term
  error = encoder.read() - final_position;
  
  //Integral term
  integral += error * deltaT;
  if(integral > 65535) integral = 65535; //Cap integral at saturation
  
  //Derivative term
  derivative = (error-prev_error)/deltaT;
  
  control_out = kp*error + ki*integral + kd*derivative;
  power = fabs(control_out);
  if(power > 65535) motor_power = 65535;
  else motor_power = power;
}
